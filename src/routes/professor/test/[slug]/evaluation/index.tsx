...
export default component$(() => {
	...
	const state = useStore({
		...
		loading: false,
		alert: false,
		points: dataResource.value.test.score.points,
		percentage: dataResource.value.test.score.points / dataResource.value.exam.points * 100,
	});

	const recalculateScore = $(() => {
		state.points = dataResource.value.test.score.tests.reduce((acc: number, curr: any) => acc + parseInt(curr.value), 0)
		dataResource.value.test.score.points = state.points
		state.percentage = state.points / dataResource.value.exam.points * 100
		dataResource.value.test.score.percentage = state.percentage.toFixed(2)
	})
		...
		<>
				...
				<div class="mx-auto max-w-7xl text-center">
					<div class="grid grid-cols-1 gap-px bg-white/5 sm:grid-cols-2 lg:grid-cols-4">
						...
						<div class="bg-indigo-900 px-4 py-6 sm:px-6 lg:px-8">
							...
							<p class="mt-2 flex items-baseline gap-x-2 justify-center">
								<span class="text-4xl font-semibold tracking-tight text-white">{state.percentage.toFixed(2)}%</span>
							</p>
						</div>
						<div class="bg-indigo-900 px-4 py-6 sm:px-6 lg:px-8">
				...
				{dataResource.value.test.score && dataResource.value.test.score.tests.map((test: any, index: any) => (
					<div key={index}>
						...
						<div class="relative mt-2 rounded-md shadow-sm">
							<input onInput$={(evt: any) => {

								dataResource.value.test.score.tests[index].value = parseInt(evt.target.value)
								dataResource.value.test.score.tests[index].passed = dataResource.value.test.score.tests[index].value > 0
								recalculateScore()
							}} type="number" name={index} id={index} class="block w-full rounded-md border-0 py-1.5 pr-10 ring-1 ring-inset focus:ring-2 focus:ring-inset focus:ring-indigo-500 sm:text-sm sm:leading-6" value={test.value} aria-invalid="true" aria-describedby="email-error" />
							<div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
								<span class="absolute px-2 text-sm bg-indigo-300 rounded-full text-white right-2">{state.examTests[index].points}</span>
							</div>
						</div>
					</div>
				))}
			...
			<div class="flex justify-center mb-4">
				<button
					...
					onClick$={async () => {
						state.loading = true
						const result: any = await TestApi.updateTestResults(dataResource.value.test._id, dataResource.value.test.score);
						state.loading = false
