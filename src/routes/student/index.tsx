...
export const onGet: RequestHandler<StudentData> = async ({ request, response, url }) => {
  ...
  if (slug !== '')
    throw response.redirect('/student/test/' + slug)
...
export default component$(() => {
  ...
  const dataResource = useEndpoint<StudentData>()
  ...
  useTask$(async () => {
    const data = await dataResource.value as StudentData
					...
					<div class="bg-white xl:w-64 xl:flex-shrink-0 xl:border-r xl:border-gray-200">
                        ...
                        {userData.user && (
                          ...
                          />
											...
											<div class="space-y-1">
												<div class="text-sm font-medium text-gray-900">{userData.user.displayName}</div>
									...
									<div class="flex flex-col space-y-6 sm:flex-row sm:space-y-0 sm:space-x-8 xl:flex-col xl:space-x-0 xl:space-y-6">
										<div class="flex items-center space-x-2">
											...
											<span class="text-sm font-medium text-gray-500">Testov: {state.tests.length}</span>
										</div>
									</div>
								</div>
							</div>
