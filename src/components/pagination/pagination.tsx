  ...
  return (
    <nav class="flex items-center justify-between border-t border-gray-200 px-4 sm:px-0">
      ...
      <div class="hidden md:-mt-px md:flex">
         {
          state.pages && Array.from(Array(state.pages), (_e, i) => {
            if(state.active === i+1) {
            return <a
              key={i}
              href={'/professor?page=' + (i+1)}
          ...
          } else if((i <= state.active && i > state.active-4) || (i > state.active && i < state.active+2)){ return (
            <a
              key={i}
              href={'/professor?page=' + (i+1)}
          ...
          <span class="inline-flex items-center pt-4 text-sm text-gray-500">
            .
          </span>
          )}
          )
      ...
      <div class="-mt-px flex w-0 flex-1 justify-end">
        <a
          href={'/professor?page=' + (state.pages)}
